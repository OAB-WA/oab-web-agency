# Google Ads Strategy for Plumber Leads Landing Page
## Complete Step-by-Step Guide ($10-15/day Budget)

**Last updated:** 2026-01-30  
*Copy this for new plumber-leads projects; rename for client.*

---

## 📊 BUDGET ANALYSIS: Is $10-15/day Realistic?

### **Short Answer: YES, but with realistic expectations**

**What $10-15/day gets you:**
- **Monthly Budget**: $300-450/month
- **Estimated Clicks/Day**: 5-15 clicks (depending on keyword competition)
- **Average CPC**: $2-5 for plumbing-related keywords
- **Expected Results**: 1-3 leads per week (with 2-4% conversion rate)

**Reality Check:**
- ✅ **Yes, you can get results** - This budget is sufficient for testing and small-scale campaigns
- ⚠️ **It's a learning budget** - First 2-4 weeks will be about optimization, not maximum conversions
- ⚠️ **Competition is HIGH** - Plumbing keywords are competitive, but your landing page is well-optimized
- ✅ **Quality over quantity** - Better to get 1 qualified lead than 10 tire-kickers

**Expected Timeline:**
- **Week 1-2**: Learning phase, Google gathering data ($0-2 leads)
- **Week 3-4**: Optimization phase, refining keywords ($1-3 leads)
- **Week 5+**: Optimized performance ($2-5 leads/week)

**ROI Potential:**
- If you convert 1 lead per week at $5,000 average project value
- Monthly cost: $300-450
- Monthly revenue potential: $20,000+ (if 1 lead converts)
- **ROI: 4,000%+ if successful**

---

## 🎯 TARGET AUDIENCE: Think Like a Plumber

### **Who You're Targeting:**
- **Primary**: Plumbing company owners (1-10 employees)
- **Secondary**: Established plumbers looking to grow
- **Pain Points**: 
  - Not getting enough calls from website
  - Losing jobs to competitors
  - Website is slow/outdated
  - Want more emergency calls
  - Need ServiceTitan/Jobber integration

### **How Plumbers Search:**
- **Direct & Practical**: "how to get more plumbing calls"
- **Problem-Focused**: "plumber website not getting leads"
- **Solution-Seeking**: "plumber website design"
- **Tool-Specific**: "ServiceTitan website integration"
- **Results-Oriented**: "increase plumbing business calls"

---

## 🔑 KEYWORD STRATEGY: Plumber Language

### **Primary Keywords (High Intent - Start Here)**

**Problem-Aware Keywords:**
- `plumber website not getting leads`
- `plumber website not getting calls`
- `how to get more plumbing leads`
- `plumber website problems`
- `why my plumbing website not working`

**Solution-Seeking Keywords:**
- `plumber website design`
- `plumber website builder`
- `website for plumbers`
- `plumber lead generation website`
- `plumber marketing website`

**Tool-Specific Keywords:**
- `ServiceTitan website integration`
- `Jobber website integration`
- `plumber website with ServiceTitan`
- `plumber website with Jobber`

**Results-Focused Keywords:**
- `get more plumbing calls`
- `increase plumbing business calls`
- `plumber website that gets calls`
- `emergency plumber website`
- `plumber website leads`

**Local + Service Keywords:**
- `plumber website design [city]` (if targeting specific areas)
- `plumber SEO services`
- `plumber website optimization`

### **Negative Keywords (CRITICAL - Add These to Avoid Wasted Spend)**

**Why Negative Keywords Matter:**
- Prevent your ads from showing on irrelevant searches
- Save budget by avoiding clicks from people who won't convert
- Improve Quality Score by reducing irrelevant traffic
- **Add these at the CAMPAIGN level** (applies to all ad groups)

**Add These Negative Keywords Immediately:**

#### **Consumer/DIY Searches (People Looking FOR Plumbers, Not Plumbers Looking for Marketing)**
- `hire`
- `find`
- `near me`
- `local`
- `emergency plumber`
- `plumber near me`
- `find a plumber`
- `best plumber`
- `plumber cost`
- `plumber price`
- `plumber rates`
- `how much does a plumber cost`
- `plumber services`
- `plumber repair`
- `plumber installation`
- `24 hour plumber`
- `emergency plumbing`
- `plumbing services`
- `plumbing repair`
- `drain cleaning`
- `water heater`
- `toilet repair`
- `pipe repair`
- `leak repair`

#### **Employment/Jobs (People Looking for Jobs, Not Marketing Services)**
- `jobs`
- `job`
- `employment`
- `career`
- `hiring`
- `apply`
- `resume`
- `plumber jobs`
- `plumbing jobs`
- `plumber career`
- `become a plumber`
- `plumber training`
- `plumber apprenticeship`
- `plumber school`

#### **Education/Training (People Wanting to Learn, Not Buy Services)**
- `training`
- `course`
- `certification`
- `certificate`
- `class`
- `learn`
- `how to become`
- `school`
- `education`
- `degree`
- `tutorial`
- `guide`
- `tips`
- `advice`

#### **DIY/How-To (Do-It-Yourself Seekers)**
- `DIY`
- `do it yourself`
- `how to`
- `how to fix`
- `how to repair`
- `how to install`
- `how to unclog`
- `how to replace`
- `step by step`
- `tutorial`
- `video`
- `instructions`

#### **Tools/Supplies/Equipment (People Buying Physical Products)**
- `tools`
- `supplies`
- `parts`
- `equipment`
- `materials`
- `pipe`
- `fittings`
- `wrench`
- `plunger`
- `drain snake`
- `pipe wrench`
- `soldering`
- `solder`
- `PVC`
- `copper pipe`
- `faucet`
- `toilet`
- `sink`
- `shower`
- `valve`

#### **Software/Apps (Unless You Want Software Buyers - Usually Not)**
- `app`
- `application`
- `mobile app`
- `iPhone app`
- `Android app`
- `software download`
- `free software`
- `plumber app`
- `plumbing app`
- `scheduling software` (unless you offer this)
- `invoicing software` (unless you offer this)

#### **Free/Cheap Seekers (Low-Value Traffic)**
- `free`
- `cheap`
- `affordable`
- `discount`
- `coupon`
- `deal`
- `bargain`
- `low cost`
- `inexpensive`
- `free trial` (unless you offer this)
- `free website` (unless you offer free websites)

#### **Unrelated Plumbing Terms**
- `plumbing code`
- `plumbing regulations`
- `plumbing license`
- `plumber license`
- `plumber exam`
- `plumber test`
- `plumbing inspection`
- `building code`
- `permit`
- `plumbing permit`

#### **Competitor/Alternative Services (If You Don't Want These)**
- `Wix`
- `Squarespace`
- `WordPress`
- `Shopify` (unless relevant)
- `template`
- `theme`
- `premade`
- `ready made`

#### **Geographic (If Not Targeting These Areas)**
- `Canada`
- `UK`
- `Australia`
- `Europe`
- Or any specific cities/states you don't serve

#### **Time-Sensitive (If Not Offering These)**
- `same day`
- `instant`
- `immediately`
- `today`
- `now` (unless your service is instant)

**How to Add Negative Keywords:**

1. **Campaign Level** (Recommended - applies to all ad groups):
   - Go to Campaign → Keywords → Negative keywords
   - Add all the keywords above
   - Use **Broad Match** for negatives (catches variations)

2. **Ad Group Level** (If you want specific negatives per group):
   - Go to Ad Group → Keywords → Negative keywords
   - Add group-specific negatives

**Pro Tip:**
- Start with the Consumer/DIY and Employment/Jobs lists first (biggest waste)
- Add more as you review your Search Terms Report weekly
- Look for irrelevant searches that got clicks and add them as negatives

**Quick Copy-Paste List (Add These First - Highest Priority):**

```
hire
find
near me
local
emergency plumber
plumber near me
find a plumber
best plumber
plumber cost
plumber price
jobs
job
employment
career
hiring
apply
training
course
certification
DIY
do it yourself
how to
how to fix
how to repair
tools
supplies
parts
equipment
app
software
free
cheap
affordable
```

**Copy this list and paste directly into Google Ads negative keywords section (one per line).**

---

## 📝 AD COPY STRATEGY: Speak Their Language

### **Ad Headline Principles:**
1. **Lead with the problem**: "Not Getting Plumbing Calls?"
2. **Promise the result**: "2-3x More Emergency Calls"
3. **Remove risk**: "Free Audit - No Obligation"
4. **Speed matters**: "1-2 Week Delivery"

### **15 Headline Variations (Use All for Best Results)**

Google Responsive Search Ads work best with 8-15 headlines. Here are 15 plumber-focused headlines covering all angles:

**Problem-Focused Headlines:**
1. `Not Getting Plumbing Calls?` (29 chars)
2. `Website Not Getting Leads?` (28 chars)
3. `Losing Calls to Competitors?` (30 chars)
4. `Website Not Working?` (22 chars)

**Results/Benefits Headlines:**
5. `Get 2-3x More Emergency Calls` (30 chars)
6. `Get More Plumbing Calls Fast` (28 chars)
7. `Websites That Get Calls` (24 chars)
8. `Increase Plumbing Calls` (25 chars)

**Tool/Integration Headlines:**
9. `ServiceTitan Integration` (26 chars)
10. `ServiceTitan & Jobber Ready` (29 chars)
11. `Jobber Website Integration` (28 chars)

**Speed/Delivery Headlines:**
12. `1-2 Week Delivery Guaranteed` (30 chars)
13. `Fast Websites for Plumbers` (26 chars)

**Free Audit/Risk Removal Headlines:**
14. `Free Audit - 24h Delivery` (28 chars)
15. `Free Audit - See Missing` (26 chars)

### **Recommended Headline Combinations by Ad Group:**

#### **Ad Group 1: Problem-Aware Keywords**
Use these 10 headlines (all 30 chars or less):
- Not Getting Plumbing Calls?
- Get 2-3x More Emergency Calls
- Free Audit - 24h Delivery
- Website Not Getting Leads?
- ServiceTitan & Jobber Ready
- 1-2 Week Delivery Guaranteed
- Websites That Get Calls
- Losing Calls to Competitors?
- Free Audit - See Missing
- Fast Websites for Plumbers

#### **Ad Group 2: Solution-Seeking Keywords**
Use these 10 headlines (all 30 chars or less):
- Get More Plumbing Calls Fast
- ServiceTitan Integration
- Free Audit - 24h Delivery
- Websites That Get Calls
- 1-2 Week Delivery Guaranteed
- Increase Plumbing Calls
- ServiceTitan & Jobber Ready
- Free Audit - See Missing
- Fast Websites for Plumbers
- Not Getting Plumbing Calls?

#### **Ad Group 3: Tool Integration Keywords**
Use these 8 headlines (all 30 chars or less):
- ServiceTitan Integration
- ServiceTitan & Jobber Ready
- Get More Plumbing Calls
- Jobber Website Integration
- Free Audit - 24h Delivery
- 1-2 Week Delivery Guaranteed
- Websites That Get Calls
- Fast Websites for Plumbers

#### **Ad Group 4: Lead Generation Keywords**
Use these 10 headlines (all 30 chars or less):
- Get 2-3x More Emergency Calls
- Get More Plumbing Calls Fast
- Increase Plumbing Calls
- Free Audit - 24h Delivery
- ServiceTitan & Jobber Ready
- 1-2 Week Delivery Guaranteed
- Websites That Get Calls
- Free Audit - See Missing
- Fast Websites for Plumbers
- Not Getting Plumbing Calls?

### **Description Variations (Use 2-3 per Ad Group - All 90 chars max):**

**Description 1: Problem-Solution** (89 chars)
```
Stop losing leads to competitors. Fast websites with ServiceTitan & Jobber integration. 1-2 week delivery. Free audit.
```

**Description 2: Results-Focused** (88 chars)
```
Plumber websites that get calls. Mobile-optimized, fast-loading, ServiceTitan ready. 1-2 week delivery. Free audit in 24h.
```

**Description 3: Urgency-Focused** (89 chars)
```
Losing emergency calls daily? Free audit shows speed issues & conversion opportunities. No obligation.
```

**Description 4: Integration-Focused** (88 chars)
```
Websites built for plumbers. ServiceTitan & Jobber integration. Mobile-optimized for emergency calls. Free audit in 24h.
```

**Description 5: Short & Direct** (78 chars)
```
Get 2-3x more plumbing calls. Fast websites with ServiceTitan integration. 1-2 week delivery. Free audit in 24 hours.
```

**Description 6: Guarantee-Focused** (87 chars)
```
Fast plumber websites that get calls. ServiceTitan & Jobber ready. 1-2 week delivery guaranteed. Free audit shows what's missing.
```

### **Ad Extensions (MUST USE):**
1. **Sitelink Extensions:**
   - "Free Website Audit"
   - "See Case Studies"
   - "ServiceTitan Integration"
   - "Get Started Today"

2. **Callout Extensions:**
   - "Free Audit in 24 Hours"
   - "1-2 Week Delivery"
   - "ServiceTitan Ready"
   - "Mobile-Optimized"
   - "No Obligation"

3. **Structured Snippets:**
   - Services: Website Design, SEO, Integration
   - Features: Fast Loading, Mobile-Friendly, Local SEO

---

## 🏗️ CAMPAIGN STRUCTURE: Step-by-Step Setup

### **Step 1: Create Campaign**
1. Go to Google Ads → Create Campaign
2. Select **"Search"** campaign type
3. Campaign Goal: **"Leads"** or **"Website traffic"**
4. Campaign Name: `Plumber Leads - Website Services`

### **Step 2: Campaign Settings**
- **Campaign Type**: Search
- **Bidding Strategy**: Start with **"Maximize Clicks"** (switch to "Maximize Conversions" after 15+ conversions)
- **Budget**: $10-15/day
- **Location**: United States (or specific states/cities)
- **Language**: English
- **Ad Schedule**: All days, all hours (optimize later based on data)

### **Step 3: Create Ad Groups (START WITH 2 - Recommended)**

**Why Start with 2?**
- Easier to manage and optimize
- Faster learning phase (Google gathers data quicker)
- Lower risk while testing
- You can add more ad groups after 2-3 weeks once you see what works

**Recommended Starting Ad Groups:**

#### **Ad Group 1: Problem-Aware Keywords** ⭐ START HERE
- **Name**: `Plumber Website Problems`
- **Why This First**: Highest intent - plumbers actively looking for solutions
- **Keywords** (Mix of high-volume + specific):
  
  **High-Volume Keywords (Add These First):**
  - `[plumber marketing]` (Exact Match)
  - `[plumber lead generation]` (Exact Match)
  - `[plumber SEO]` (Exact Match)
  - `"plumber marketing"` (Phrase Match)
  - `"plumber lead generation"` (Phrase Match)
  - `"plumber website marketing"` (Phrase Match)
  - `+plumber +marketing` (Broad Match Modified)
  - `+plumber +lead +generation` (Broad Match Modified)
  
  **Specific Problem Keywords (Keep These - May Show "Low Volume" Initially):**
  - `[plumber website not getting leads]` (Exact Match)
  - `[plumber website not getting calls]` (Exact Match)
  - `"plumber website problems"` (Phrase Match)
  - `"plumber website not generating leads"` (Phrase Match)
  - `+plumber +website +not +getting +leads` (Broad Match Modified)
  
  **Note**: "Low search volume" keywords are fine to keep - they'll become eligible when search volume increases, and they're highly targeted.
  
- **Headlines**: Use Ad Group 1 headlines from section above (10 headlines)
- **Descriptions**: Use Description 1 (Problem-Solution) and Description 3 (Urgency-Focused)

#### **Ad Group 2: Solution-Seeking Keywords** ⭐ START HERE
- **Name**: `Plumber Website Design`
- **Why This Second**: High volume, people ready to buy
- **Keywords** (10-12 keywords):
  - `[plumber website design]` (Exact Match)
  - `[website for plumbers]` (Exact Match)
  - `"plumber website builder"` (Phrase Match)
  - `"plumber website development"` (Phrase Match)
  - `"plumber website services"` (Phrase Match)
  - `"plumber website company"` (Phrase Match)
  - `+plumber +website +design` (Broad Match Modified)
  - `+plumber +website +builder` (Broad Match Modified)
- **Headlines**: Use Ad Group 2 headlines from section above (10 headlines)
- **Descriptions**: Use Description 2 (Results-Focused) and Description 4 (Integration-Focused)

**Add These Later (After 2-3 Weeks):**

#### **Ad Group 3: Tool Integration Keywords** (Add Week 3-4)
- **Name**: `ServiceTitan Jobber Integration`
- **Keywords**:
  - `[ServiceTitan website integration]` (Exact Match)
  - `[Jobber website integration]` (Exact Match)
  - `"plumber website with ServiceTitan"` (Phrase Match)
  - `+ServiceTitan +website +plumber` (Broad Match Modified)
- **Headlines**: Use Ad Group 3 headlines from section above (8 headlines)
- **Descriptions**: Use Description 4 (Integration-Focused)

#### **Ad Group 4: Lead Generation Keywords** (Add Week 3-4)
- **Name**: `Get More Plumbing Calls`
- **Keywords**:
  - `[get more plumbing calls]` (Exact Match)
  - `"increase plumbing business calls"` (Phrase Match)
  - `"plumber website that gets calls"` (Phrase Match)
  - `+plumber +website +leads` (Broad Match Modified)
- **Headlines**: Use Ad Group 4 headlines from section above (10 headlines)
- **Descriptions**: Use Description 3 (Urgency-Focused)

### **Step 4: Keyword Match Types Explained**
- **Exact Match [keyword]**: Only shows for exact search
- **Phrase Match "keyword"**: Shows when phrase is included
- **Broad Match Modified +keyword**: Shows for variations with + words

**Start Strategy**: 70% Exact/Phrase, 30% Broad Modified (to discover new terms)

### **Step 4.5: Handling "Low Search Volume" Keywords**

**What "Low Search Volume" Means:**
- Google won't show ads for these keywords because there aren't enough monthly searches
- This is common with very specific, long-tail keywords
- **Don't delete them!** They'll become eligible when search volume increases

**What To Do:**
1. **Keep the specific keywords** - They're highly targeted and will convert well when eligible
2. **Add broader keywords** - Add higher-volume terms to get impressions immediately:
   - Instead of just "plumber website not getting leads"
   - Also add: "plumber marketing", "plumber lead generation", "plumber SEO"
3. **Use Broad Match Modified** - This helps discover variations:
   - `+plumber +website +problems` will match many related searches
4. **Check back weekly** - Some "low volume" keywords become eligible as search trends change

**Recommended Fix for Your Current Ad Group:**
Add these higher-volume keywords to get impressions:
- `[plumber marketing]`
- `[plumber lead generation]`
- `[plumber SEO]`
- `"plumber marketing"`
- `"plumber lead generation"`
- `+plumber +marketing`
- `+plumber +lead +generation`

These will get impressions while your specific keywords wait for eligibility.

  ### **Step 5: Landing Page URL & UTM Tracking**

#### **Final URL Setup:**
- **Base URL**: `https://www.oabwebagency.com/plumber-leads`
- **Important**: Use the exact landing page URL (no redirects if possible)

#### **UTM Parameters (Why They Matter):**
UTM parameters help you track which ads/keywords are driving conversions in Google Analytics. This is separate from Google Ads conversion tracking but provides valuable insights.

#### **UTM Parameter Structure:**
```
?utm_source=google
&utm_medium=cpc
&utm_campaign=plumber_leads
&utm_content={ad_group_name}
&utm_term={keyword}
```

#### **For Your Campaign, Use:**

**Ad Group 1 (Plumber Website Problems):**
```
https://www.oabwebagency.com/plumber-leads?utm_source=google&utm_medium=cpc&utm_campaign=plumber_leads&utm_content=plumber_website_problems
```

**Ad Group 2 (Plumber Website Design):**
```
https://www.oabwebagency.com/plumber-leads?utm_source=google&utm_medium=cpc&utm_campaign=plumber_leads&utm_content=plumber_website_design
```

#### **How to Add UTM Parameters in Google Ads:**
1. When creating ads, in the "Final URL" field, enter your base URL
2. Click "Track URL" or "Use tracking template" (if available)
3. Google Ads can auto-add parameters, OR
4. Manually add the full URL with UTM parameters

#### **Alternative: Use Google Ads Tracking Template (Recommended)**
Instead of adding UTM to each ad, set up a tracking template at the campaign level:

1. Go to Campaign Settings → Additional Settings
2. Find "Tracking template" or "URL options"
3. Add: `{lpurl}?utm_source=google&utm_medium=cpc&utm_campaign={campaign}&utm_content={adgroup}`
4. This automatically adds UTM parameters to all ads in the campaign

#### **UTM Parameter Breakdown:**
- **utm_source=google**: Identifies Google as traffic source
- **utm_medium=cpc**: Identifies paid search (cost-per-click)
- **utm_campaign=plumber_leads**: Your campaign name
- **utm_content={ad_group}**: Which ad group (auto-filled by Google Ads)
- **utm_term={keyword}**: Which keyword triggered the ad (optional, auto-filled)

#### **Viewing UTM Data:**
- Check Google Analytics → Acquisition → Campaigns
- See which ad groups/keywords drive the most conversions
- Compare with Google Ads data for cross-validation

---

### **Step 6: Conversion Tracking Setup (CRITICAL)**

**Why This Is Essential:**
- Google Ads can't optimize without conversion data
- You won't know which keywords/ads are profitable
- Can't use automated bidding strategies effectively
- Wastes budget on non-converting keywords

#### **Option 1: Google Ads Conversion Tracking (Recommended for Beginners)**

**Step-by-Step Setup:**

1. **Create Conversion Action:**
   - Go to Google Ads → Tools & Settings → Conversions
   - Click "+" to create new conversion
   - Select "Website" as conversion source
   - Choose "Submit lead form" as conversion category
   - Name it: "Plumber Leads - Form Submission"

2. **Conversion Settings:**
   - **Category**: Lead
   - **Value**: 
     - Choose "Use different values for each conversion"
     - Set default value: $100-500 (estimate based on average project value)
     - Or use "Don't use a value" if unsure
   - **Count**: "One" (each form submission = 1 conversion)
   - **Click-through window**: 30 days (how long to attribute conversions)
   - **View-through window**: 1 day (optional, for display ads)

3. **Install Conversion Tag:**
   - Google will provide a conversion tracking code
   - Two options:
   
   **Option A: Google Tag (Recommended - Easier)**
   - Install Google Tag Manager on your site
   - Add conversion tag through GTM
   - More flexible for future tracking
   
   **Option B: Direct Installation**
   - Copy the conversion tracking code
   - Add to your landing page (in `<head>` section)
   - Or add to form success/thank you page

4. **For Your Plumber Leads Page:**
   - Add conversion tag to the form success page/state
   - Or trigger on form submission success message
   - Test by submitting the form and checking if conversion fires

5. **Verify It's Working:**
   - Submit a test form submission
   - Go to Google Ads → Conversions
   - Should see conversion within 24-48 hours (or use Google Tag Assistant to verify immediately)

#### **Option 2: Enhanced Conversions (Recommended for Better Data)**

**What It Is:**
- Sends hashed customer data (email, phone) to Google
- Improves conversion tracking accuracy
- Better for privacy-compliant tracking
- Helps with iOS 14.5+ tracking limitations

**How to Set Up:**
1. In conversion action settings, enable "Enhanced conversions"
2. Add customer data collection to your form submission
3. Google will hash and send: email, phone number, name
4. Improves match rate and conversion tracking accuracy

#### **Option 3: Google Analytics 4 (GA4) Import (Advanced)**

**If You Already Have GA4:**
1. Set up GA4 event for form submissions
2. Link GA4 to Google Ads
3. Import GA4 conversions into Google Ads
4. Can use both GA4 and Google Ads tracking simultaneously

**For Your Landing Page:**
- You already have GA4 tracking (based on your code)
- Event: `conversion_event_submit_lead_form_1`
- Can import this into Google Ads as conversion

#### **Conversion Value Strategy:**

**How to Set Conversion Value:**
- **Conservative**: $100 (low estimate, safe for optimization)
- **Realistic**: $250-500 (based on average project value)
- **Optimistic**: $500-1000 (if you know your close rate)

**Example Calculation:**
- If 1 in 10 leads converts to $5,000 project
- Average lead value = $5,000 ÷ 10 = $500
- Set conversion value to $500

**Why Value Matters:**
- Google optimizes for highest-value conversions
- Helps with "Maximize Conversion Value" bidding
- Better ROI tracking

#### **Testing Your Conversion Tracking:**

1. **Before Launch:**
   - Submit a test form
   - Check Google Tag Assistant (Chrome extension)
   - Verify conversion tag fires
   - Check Google Ads → Conversions (may take 24-48 hours)

2. **After Launch:**
   - Monitor conversions daily for first week
   - Compare with actual form submissions
   - If mismatch, troubleshoot tracking code
   - Verify UTM parameters are working in GA4

#### **Common Issues & Fixes:**

**Issue**: Conversions not showing up
- **Fix**: Check if tag is installed correctly, verify form submission triggers tag

**Issue**: Duplicate conversions
- **Fix**: Check "Count" setting (should be "One"), verify tag isn't firing twice

**Issue**: Conversions delayed
- **Fix**: Normal - can take 3-24 hours to appear in Google Ads

**Issue**: Wrong conversion value
- **Fix**: Update conversion action settings, or use dynamic value passing

#### **Best Practices:**
- ✅ Set up conversion tracking BEFORE launching ads
- ✅ Test with a real form submission
- ✅ Use Enhanced Conversions for better accuracy
- ✅ Set realistic conversion values
- ✅ Monitor daily for first week
- ✅ Link GA4 for additional insights
- ✅ Don't change conversion settings after launch (unless necessary)

#### **For Your Specific Landing Page:**

Based on your code, you already have:
- GA4 event tracking: `conversion_event_submit_lead_form_1`
- Form submission tracking via Formspark

**Recommended Setup:**
1. Set up Google Ads conversion tracking (separate from GA4)
2. Add conversion tag to form success state
3. Enable Enhanced Conversions (send email/phone data)
4. Link GA4 to Google Ads and import conversions
5. Use both for comprehensive tracking

**This gives you:**
- Google Ads native conversion tracking (for optimization)
- GA4 event tracking (for detailed analytics)
- Enhanced Conversions (for better accuracy)
- Complete visibility into what's working

---

## 📈 OPTIMIZATION STRATEGY: Week-by-Week

### **Week 1-2: Learning Phase**
- **Goal**: Let Google gather data
- **Actions**:
  - Don't make major changes
  - Monitor search terms report daily
  - Add negative keywords for irrelevant searches
  - Check which ads are performing
- **Expected**: 0-2 leads (this is normal!)

### **Week 3-4: Optimization Phase**
- **Goal**: Refine based on data
- **Actions**:
  - Pause low-performing keywords (0 clicks after 100+ impressions)
  - Increase bids on high-performing keywords
  - Test new ad variations
  - Add more negative keywords
  - Adjust ad schedule if you see time patterns
- **Expected**: 1-3 leads

### **Week 5+: Scaling Phase**
- **Goal**: Maximize conversions
- **Actions**:
  - Switch to "Maximize Conversions" bidding (if 15+ conversions)
  - Double down on winning keywords
  - Expand similar keywords
  - Test landing page variations
  - Consider increasing budget if profitable
- **Expected**: 2-5 leads/week

---

## 🎯 CONVERSION OPTIMIZATION: Match Ad to Landing Page

### **Ad-to-Landing Page Alignment:**
Your landing page is **excellent** for conversions. Ensure ads match:

1. **Ad Promise = Landing Page Headline**
   - Ad: "Get 2-3x More Emergency Calls"
   - Landing Page: "Get 2-3x More Emergency Calls" ✅ Perfect match!

2. **Ad CTA = Landing Page CTA**
   - Ad: "Free Audit"
   - Landing Page: "Get Your Free Audit" ✅ Perfect match!

3. **Ad Benefits = Landing Page Benefits**
   - Ad mentions: ServiceTitan, Jobber, Fast delivery
   - Landing Page: All mentioned ✅ Perfect match!

### **Landing Page Strengths for Ads:**
- ✅ Clear value proposition
- ✅ Strong headline
- ✅ Multiple CTAs
- ✅ Trust signals (guarantees)
- ✅ Fast loading (important for Quality Score)

### **Quick Wins Before Launching Ads:**
Based on your conversion analysis, fix these first:
1. Make phone field required (5 min fix)
2. Add "Secure" badge to form (10 min fix)
3. Change button to "Get My Free Audit Now" (2 min fix)

---

## 💰 BIDDING STRATEGY: Start Smart

### **Initial Bidding:**
- **Strategy**: Manual CPC (Maximum Cost Per Click)
- **Start Bids**: $2-4 per keyword (adjust based on competition)
- **Monitor**: First week, adjust daily

### **Bid Adjustments:**
- **Mobile**: +20% (your landing page is mobile-optimized)
- **Desktop**: 0% (baseline)
- **Tablet**: -10% (lower conversion rate typically)

### **When to Switch Bidding:**
- **After 15+ conversions**: Switch to "Maximize Conversions"
- **After 30+ conversions**: Consider "Target CPA" (Cost Per Acquisition)

---

## 📊 TRACKING & MEASUREMENT: What to Monitor

### **Daily Metrics:**
- Impressions
- Clicks
- CTR (Click-Through Rate) - Target: 2-5%
- Average CPC
- Conversions (form submissions)
- Cost per conversion

### **Weekly Review:**
- Which keywords are converting?
- Which ads are performing best?
- What search terms are triggering ads?
- What's the cost per lead?
- What's the conversion rate?

### **Key Performance Indicators (KPIs):**
- **Target CTR**: 2-5%
- **Target CPC**: $2-5
- **Target Conversion Rate**: 2-4%
- **Target Cost Per Lead**: $20-50
- **Target ROI**: 10x+ (if average project is $5,000)

---

## 🚨 COMMON MISTAKES TO AVOID

1. **❌ Too Many Keywords**: Start with 10-15 keywords per ad group
2. **❌ Ignoring Negative Keywords**: Add them daily from search terms report
3. **❌ Not Testing Ads**: Always have 2-3 ad variations per ad group
4. **❌ Changing Too Fast**: Give campaigns 1-2 weeks before major changes
5. **❌ Ignoring Mobile**: 60%+ of searches are mobile - optimize for it
6. **❌ Not Tracking Conversions**: Can't optimize without data
7. **❌ Broad Match Only**: Use exact/phrase match for control
8. **❌ Generic Ad Copy**: Use plumber-specific language

---

## 📋 PRE-LAUNCH CHECKLIST

### **Before You Launch:**
- [ ] Landing page is live and working
- [ ] Form submission tracking is set up
- [ ] Google Ads conversion tracking is installed
- [ ] UTM parameters are configured
- [ ] At least 2 ad variations per ad group
- [ ] Ad extensions are set up (sitelinks, callouts)
- [ ] Negative keywords list is ready
- [ ] Budget is set ($10-15/day)
- [ ] Bidding strategy is chosen
- [ ] Landing page matches ad promises

---

## 🎯 EXPECTED RESULTS TIMELINE

### **Month 1:**
- **Spend**: $300-450
- **Clicks**: 60-150
- **Leads**: 2-6
- **Cost per Lead**: $50-150
- **Focus**: Learning and optimization

### **Month 2:**
- **Spend**: $300-450
- **Clicks**: 80-200
- **Leads**: 4-10
- **Cost per Lead**: $30-75
- **Focus**: Refining winners

### **Month 3+ (Optimized):**
- **Spend**: $300-450
- **Clicks**: 100-250
- **Leads**: 8-20
- **Cost per Lead**: $15-40
- **Focus**: Scaling profitable keywords

---

## 💡 PRO TIPS FOR SUCCESS

1. **Start Small**: 3-4 ad groups, 10-15 keywords each
2. **Be Patient**: First 2 weeks are learning phase
3. **Monitor Daily**: Check search terms, add negatives
4. **Test Constantly**: New ad copy, new keywords
5. **Focus on Quality**: Better to get 1 good lead than 10 bad ones
6. **Use Plumber Language**: "Emergency calls", "ServiceTitan", "Get more jobs"
7. **Match Landing Page**: Ad promise = landing page delivery
8. **Track Everything**: Can't improve what you don't measure

---

## 🔄 SCALING STRATEGY (After Success)

Once you're getting 2-3 leads/week consistently:

1. **Increase Budget**: $15/day → $25/day → $50/day
2. **Expand Keywords**: Add similar terms that are working
3. **New Ad Groups**: Test new angles (local SEO, speed optimization)
4. **Landing Page Split Tests**: Test different headlines, CTAs
5. **Remarketing**: Target people who visited but didn't convert
6. **Display/Video**: Expand to other Google networks

---

## 📞 NEXT STEPS

1. **Review this guide** - Understand the strategy
2. **Set up Google Ads account** - If you don't have one
3. **Install conversion tracking** - Critical for success
4. **Create campaign structure** - Follow Step 3 above
5. **Write ad copy** - Use samples provided
6. **Set budget** - Start at $10-15/day
7. **Launch** - Start with 3-4 ad groups
8. **Monitor daily** - Check search terms, add negatives
9. **Optimize weekly** - Pause losers, boost winners
10. **Scale when profitable** - Increase budget gradually

---

## ✅ FINAL RECOMMENDATION

**Your landing page is well-optimized for conversions.** With a $10-15/day budget, you can realistically expect:
- **1-3 leads per week** after optimization (weeks 3-4)
- **Cost per lead: $20-50** (very reasonable for this industry)
- **ROI potential: 4,000%+** if you convert 1 lead per month

**The key is patience and optimization.** Don't expect miracles in week 1, but by month 2-3, you should have a profitable campaign.

**Start with the Problem-Aware and Solution-Seeking ad groups first** - these will have the highest conversion rates.

---

## After you make updates

Whenever you change the Plumbers campaign (keywords, bids, ads, budgets), use this rhythm.

**What to expect after these updates**
- More impressions and possibly more clicks after adding keywords or broadening match.
- Campaign diagnostics may clear some warnings after 1–2 weeks.
- Cost per click and cost per lead may shift; give it 1–2 weeks before judging.

**When to check for results**
- **First check:** 3–5 days after the change (see if impressions/clicks moved).
- **Meaningful check:** 7–14 days (enough data to compare; fill in *Result* in Document history if you want).
- **Ongoing:** Check Search terms, negatives, and cost per lead weekly (see GOOGLE_ADS_FREE_AUDIT_3_CAMPAIGNS section 9).

**What to do next**
- Re-check **Campaign diagnostics**; if "Targeting fewer searches" / "Not targeting relevant searches" are gone, consider broad match for top keywords.
- Once you have ~30 conversions, consider Target CPA instead of Maximize conversions.
- Update this doc’s **Document history** (and CHANGELOG_GOOGLE_ADS.md) with the change and, if you like, the *Result* after 1–2 weeks.

---

*This strategy is designed specifically for your plumber leads landing page. Adjust based on your actual results and data.*

---

## Document history

- **2026-01-30** – Added **After you make updates** section (What to expect | When to check | What to do next). *Why:* Guide should show what to expect after changes, when to check results, and what to do next for reuse on new projects. *Scope:* Plumbers strategy doc only.
- **2026-01-30** – Synced keyword list with GOOGLE_ADS_FREE_AUDIT_3_CAMPAIGNS (added website/audit + SEO/leads terms to 3-campaign doc; this doc remains the deep-dive reference). *Why:* Single source of truth for Plumbers campaign; diagnostics drove keyword expansion. *Scope:* Plumbers strategy doc only.
